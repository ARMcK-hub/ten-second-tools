...
class AsyncPoolExecutor(ABC):
    ...
    def __init__(
        self, num_workers: int = os.cpu_count(), silent: Optional[bool] = True
    ) -> None:
    ...
    def execute(self, job: AsyncJob) -> None:
        ...
        with pool_executor(max_workers=self.num_workers) as executor:
            ...
            for i in range(self.num_workers):
                futures.append(
                    executor.submit(
                        self.__queued_process_loop, job.executable, job.process_queue
                    )
                )
        ...
        if not self.silent:
            print(
                f"Job ({job.id}) End Time: {job.end_time}          (Duration: {job.duration})"
            )
